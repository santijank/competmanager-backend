          ...
          <div className="grid grid-cols-3 gap-4 pt-4 border-t border-gray-200">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600">
                {sortedRegistrations.filter(r => r.final_rank && r.final_rank <= 3).length}
              </div>
              <div className="text-sm text-gray-600">รางวัล Top 3</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-600">
                {sortedRegistrations[0]?.final_score?.toFixed(2) || '-'}
              </div>
          ...
          {sortedRegistrations.length === 0 ? (
            ...
            <div className="divide-y divide-gray-100">
              {sortedRegistrations.map((reg, index) => (
                <div
                  key={reg.id}
                  className={`p-5 hover:bg-gray-50 transition-colors border-l-4 ${getMedalColor(reg.final_rank)}`}
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-4 flex-1">
                      ...
                      <div className="text-center w-16">
                        {reg.final_rank ? (
                          <>
                            <div className="text-3xl">{getMedalIcon(reg.final_rank)}</div>
                            <div className="text-sm font-semibold text-gray-700">
                              อันดับ {reg.final_rank}
                            </div>
                          </>
                        ) : (
