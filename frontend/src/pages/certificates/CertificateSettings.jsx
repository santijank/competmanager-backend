...
import { Save, Upload, Trash2, Image, PenLine, Eye } from 'lucide-react';
...
export default function CertificateSettings() {
  ...
  const fetchSettings = async () => {
    try {
      setLoading(true);
      const response = await api.get('/system-settings/certificate');
  ...
  const handleSave = async () => {
    try {
      ...
      setRemoveBackground(false);
      signers.forEach((s, i) => {
        signers[i].signatureFile = null;
        signers[i].removeSignature = false;
      });
      fetchSettings();
    } catch (error) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-1">กำหนดภาพพื้นหลัง ชื่อผู้บริหาร และลายเซ็นสำหรับเกียรติบัตร</p>
      </div>
