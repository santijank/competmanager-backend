...
<head>
    ...
    <title>แบบลงทะเบียนครู (รวม)</title>
    <style>
        ...
        .col-teacher { width: 30%; text-align: left; padding-left: 5px; }
        .col-school { width: 27%; text-align: left; padding-left: 5px; }
        .col-affiliation { width: 15%; text-align: center; }
        .col-signature { width: 20%; text-align: left; padding-left: 6px; }
...
@foreach($allCompetitionsData as $index => $compData)
    ...
    @if(isset($isAllCategories) && $isAllCategories && $competition->category_id !== $currentCategoryId)
        ...
        <div class="category-separator">
            ...
            <div class="category-separator-sub">เอกสารลงทะเบียนครูผู้ฝึกสอน (DC.02)</div>
        </div>
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            @forelse($schools as $schoolData)
                @php $teachers = $schoolData['teachers'] ?? []; $schoolName = $schoolData['school_name'] ?? '-'; @endphp
                @if(count($teachers) > 0)
                ...
                @endif
            @empty
                <tr><td colspan="5" style="text-align: center; padding: 20px;">ไม่มีข้อมูลครูผู้ฝึกสอน</td></tr>
            @endforelse
        </tbody>
    </table>
@endforeach
