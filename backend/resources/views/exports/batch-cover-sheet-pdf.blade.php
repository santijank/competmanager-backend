...
<head>
    ...
    <title>ใบปะหน้าซองเอกสาร (รวม)</title>
    <style>
        ...
        @page { margin: 0; }
        body { font-family: 'THSarabunNew', sans-serif; font-size: 14pt; line-height: 1.2; margin: 0; padding: 20mm 15mm 20mm 18mm; }
        ...
        .info-cell { text-align: left; padding-left: 8px; vertical-align: middle; }
        .header-text { font-size: 14pt; font-weight: bold; line-height: 1.3; }
        .header-text-normal { font-size: 14pt; line-height: 1.3; }
        .header-text-green { font-size: 14pt; font-weight: bold; color: #006600; line-height: 1.3; }
        .activity-box { border: 2px solid #006600; padding: 10px; margin: 8px 0; text-align: center; }
        .activity-code { font-size: 14pt; font-weight: bold; margin-bottom: 3px; }
        .activity-name { font-size: 14pt; color: #006600; margin-bottom: 3px; }
        .activity-datetime { font-size: 14pt; color: #cc0000; margin-bottom: 3px; }
        .activity-venue { font-size: 14pt; }
        .instructions { margin-top: 10px; }
        .instructions-title { font-size: 14pt; font-weight: bold; text-decoration: underline; margin-bottom: 6px; }
        .instruction-list { font-size: 13pt; line-height: 1.3; padding-left: 5px; }
        .instruction-item { margin-bottom: 4px; }
        .sub-list { padding-left: 25px; margin-top: 2px; }
...
@foreach($allCompetitionsData as $index => $compData)
    @php
        ...
        $competitionDateText = ''; $competitionTimeText = '';
        $thaiMonths = ['', 'มกราคม', 'กุมภาพันธ์', 'มีนาคม', 'เมษายน', 'พฤษภาคม', 'มิถุนายน', 'กรกฎาคม', 'สิงหาคม', 'กันยายน', 'ตุลาคม', 'พฤศจิกายน', 'ธันวาคม'];
        $dateToUse = null;
        if (isset($schedule) && $schedule && $schedule->competition_date) { $dateToUse = $schedule->competition_date; if ($schedule->start_time) { $competitionTimeText = "เวลา " . \Carbon\Carbon::parse($schedule->start_time)->format('H:i') . " น."; } }
        elseif ($competition->competition_date) { $dateToUse = $competition->competition_date; }
        if ($dateToUse) { $day = $dateToUse->format('j'); $month = $thaiMonths[(int)$dateToUse->format('n')]; $rawYear = (int) $dateToUse->format('Y'); $year = $rawYear > 2400 ? $rawYear : $rawYear + 543; $competitionDateText = "วันที่ {$day} {$month} พ.ศ.{$year}"; }
        $venueAndDate = '';
    ...
    <div class="instructions">
        ...
        <div class="instruction-list">
            ...
            <div class="instruction-item"><strong>3. ผู้ฝึกสอนลงทะเบียน</strong> ให้ผู้ฝึกสอนเขียนชื่อคนอย่างตัวบรรจง และให้ตรวจสอบ ชื่อ-สกุล</div>
            <div class="instruction-item">
            ...
            <div class="instruction-item"><strong>6. นำเอกสารทั้งหมดใส่ซองส่งคณะดำเนินงาน</strong> นำส่งคณะดำเนินงานการแข่งขันฝ่ายบันทึกผลและรายงานผลการแข่งขันต่อไป</div>
        </div>
    </div>
@endforeach
</body>
