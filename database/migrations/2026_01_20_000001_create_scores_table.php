...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('scores', function (Blueprint $table) {
            ...
            // ผู้ใส่คะแนนและเวลา
            $table->foreignId('scored_by')->nullable()->constrained('users')->onDelete('set null')->comment('ผู้ใส่คะแนน');
            ...
            $table->foreignId('finalized_by')->nullable()->constrained('users')->onDelete('set null')->comment('ผู้ล็อคคะแนน');
            $table->timestamp('finalized_at')->nullable()->comment('วันเวลาที่ล็อค');
            
            // Advancement (ส่งตัวต่อ)
            ...
            // Unique constraint: แต่ละ registration มีคะแนนได้แค่ 1 รายการ
            $table->unique('registration_id', 'unique_registration_score');
        });
    }
